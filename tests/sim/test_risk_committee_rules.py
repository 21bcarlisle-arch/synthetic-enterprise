...
def test_parse_handshake_triggered_list_order():
    ctx = "Recommendation requested: adjust hedge_fraction for C5, C7, C9"
    parsed = parse_handshake(ctx)
    assert parsed["triggered_customers"] == ["C5", "C7", "C9"]


def test_adjustment_step_boundary_2p5():
    from sim.risk_committee_rules import _adjustment_step
    assert _adjustment_step(2.5) == pytest.approx(0.20)


def test_adjustment_step_boundary_3p0():
    from sim.risk_committee_rules import _adjustment_step
    assert _adjustment_step(3.0) == pytest.approx(0.25)


def test_crisis_sigma_threshold_is_1p5():
    assert CRISIS_SIGMA_THRESHOLD == pytest.approx(1.5)
