...
def test_MUTATION_a_class_recurring_in_ANOTHER_lane_still_counts_as_recurring(tmp_path):
    """THE CASE, found the day this module landed and by its own output.

    `measurements_that_mirror` was recorded ACCEPTED at 7 instances on the grounds that it had
    stopped recurring — 1 instance in the trailing week. By that evening it had THREE more, all
    filed that day, all `W2_customer_generator` against an `H_harness` register. The lane guard
    correctly REFUSED to consolidate them (severity is lane-scoped; filing a W2 finding under
    H_harness would launder W2's own blocker), and this module read the same-lane count and
    reported "not accruing" for a class that had recurred three times in a day.

    The lane guard is right and stays. What was wrong was reading it as the POPULATION.
    Consolidation is a claim about SUPERSESSION and must not cross a lane; accrual and cost are
    claims about HOW OFTEN THE SHAPE HAPPENS, which have no lane. A class register that cannot see
    its own class recurring one lane over is a control whose subject moved — which is the shape
    these registers exist to catalogue, committed by the instrument that ranks them."""
    _register(tmp_path)
    # Three instances of the class, in a DIFFERENT lane from the register's H_harness.
    for i in range(3):
        path = tmp_path / f"WORKER_FINDING_A_CONTROL_HAS_NO_CALLER_X{i}_2026-08-30.md"
        path.write_text(
            f"# [WORKER FINDING] no caller {i}\n\n"
            "**Severity:** LATENT · **Lane:** W2_customer_generator\n",
            encoding="utf-8",
        )
    debt = _debt(tmp_path)
    assert debt.out_of_lane == 3, "the refused instances must be reported, not silently dropped"
    assert debt.instances == 3, "an out-of-lane instance is still an instance of the class"
    assert debt.still_accruing, (
        "a class that produced three instances in one day is accruing, whatever lane they carry — "
        "reading the same-lane count as the population is how a register goes quiet on itself"
    )


def test_MUTATION_an_acceptance_is_overtaken_by_out_of_lane_instances_too(tmp_path):
    """The half that has teeth. If out-of-lane instances did not count, an ACCEPTED decision could
    never be overtaken by them — the class would recur indefinitely one lane over while its
    register said the matter was settled, which is a silencer with a lane guard for an alibi."""
    _register(tmp_path, disposition=(
        "## Disposition\n\n**Decision:** ACCEPTED\n**Taken:** 2026-09-01\n"
        "**At:** 1 instances\n**Because:** it had stopped recurring\n"
    ))
    for i in range(3):
        path = tmp_path / f"WORKER_FINDING_A_CONTROL_HAS_NO_CALLER_Y{i}_2026-08-30.md"
        path.write_text(
            f"# [WORKER FINDING] no caller {i}\n\n"
            "**Severity:** LATENT · **Lane:** W2_customer_generator\n",
            encoding="utf-8",
    ...
    debt = _debt(tmp_path)
    assert debt.drawable, "an acceptance no out-of-lane evidence can overtake is a silencer"
    assert "re-opened" in debt.draw_verdict()[1]
