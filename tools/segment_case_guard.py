...
exact spelling is not the canonical one, appearing in either:
  ...
  2. a module-level constant whose NAME mentions SEGMENT -- which is how
     `_IC_SEGMENTS = ("ic", "I&C")` re-enters through the back door.

Canonical spellings are not flagged: `seg == "resi"` is correct today. It is
still better written `normalise_segment(seg) == RESIDENTIAL`, but this guard
enforces CORRECTNESS, not style -- a guard that also fired on correct code
...
from simulation.segment_vocabulary import _ALIASES
    ...
    def _check(self, node: ast.AST) -> None:
        ...
        for child in ast.walk(node):
            ...
            if canonical is not None and raw != canonical:
                self.violations.append((child.lineno, raw, canonical))
...
def scan(root: Path, repo_root: Path) -> tuple[list[str], int]:
    ...
    for path in sorted(root.rglob("*.py")):
        ...
        if rel in EXEMPT:
            continue
        scanned += 1
        for lineno, raw, canonical in scan_file(path, repo_root):
            messages.append(
                "%s:%d: segment literal %r is not canonical -- use %r "
                "(or better, simulation.segment_vocabulary.normalise_segment)"
                % (rel, lineno, raw, canonical)
            )
