...
def extract_report_data(run_output: dict) -> dict:
    ...
    return {
        ...
        "feedback_survey_log": phase2b.get("feedback_survey_log", []),
        "reputation_events_log": phase2b.get("reputation_events_log", []),
        "nps_annual_summaries": phase2b.get("nps_annual_summaries", {}),
        "complaint_annual_summaries": phase2b.get("complaint_annual_summaries", {}),
        "gri_trajectory": phase2b.get("gri_trajectory", []),
        ...
        # collection book (mandates + attempts + summary) -- the wiring that
        ...
        "contact_centre_log": run_output.get("contact_centre_log", []),
        # Phase PZ: scenario stress testing (loaded from disk — generated by run_live_decisions)
        "scenario_analysis": _load_scenario_analysis(),
    }


def _load_scenario_analysis() -> dict:
    """Load scenario_analysis_latest.json if available (written by run_live_decisions)."""
    path = _PROJECT / "site" / "state" / "scenario_analysis_latest.json"
    try:
...
# MOVED, 2026-08-19. The population step now lives in
# `saas/reporting/compliance_scorecard_population.py` so that a RENDERING is not the
# home of a computation other surfaces need. Re-exported here rather than deleted:
# `_section_compliance_scorecard` below still calls it, and deleting the name would
# break importers for no gain. The production importer
# (tools/generate_dashboard_data.py) now takes it from the new module directly, which
# is what `tools/annual_report_import_ratchet.py` freezes at zero.
from saas.reporting.compliance_scorecard_population import (  # noqa: F401
...
def _section_scenario_sensitivity(data: dict) -> str:
    """Phase PZ: Scenario Sensitivity Analysis — live portfolio under base/bull/bear/crisis."""
    sa = data.get("scenario_analysis", {})
    if not sa:
        return "## Scenario Sensitivity Analysis (Phase PZ)\n\n_Not available: run process_run_complete to generate._\n"

    scenarios = sa.get("scenarios", {})
    exposure_delta = sa.get("portfolio_exposure_delta_gbp", {})
    active_customers = sa.get("active_customers", 0)
    projection_months = sa.get("projection_months", 12)
    generated_at = sa.get("generated_at", "unknown")

    lines = [
        "## Scenario Sensitivity Analysis (Phase PZ)",
        "",
        f"Live portfolio ({active_customers} active customers) under {projection_months}-month forward scenarios.",
    ...
    _SCENARIO_ORDER = ["base", "bull", "bear", "crisis"]
    lines.append("| Scenario | Elec Fwd (£/MWh) | Gas Fwd (£/MWh) | Hedge Rec | Renewing | Exposure Delta |")
    lines.append("|----------|------------------|-----------------|-----------|----------|----------------|")
    for name in _SCENARIO_ORDER:
        ...
        elec_fwd = s.get("elec_12m_forward_gbp_per_mwh", 0)
        gas_fwd = s.get("gas_12m_forward_gbp_per_mwh", 0)
        hedge_rec = s.get("hedge_recommendation", "?")
        n_renewing = len(s.get("renewal_flags", []))
        if name == "base":
            delta_str = "—"
        else:
            delta = exposure_delta.get(name, 0.0)
            sign = "+" if delta >= 0 else ""
            delta_str = f"{sign}£{delta:,.0f}"
        lines.append(f"| {label} | {elec_fwd:.1f} | {gas_fwd:.1f} | {hedge_rec} | {n_renewing} | {delta_str} |")
    lines.append("")

    lines.append("**Scenario labels:**")
    for name in _SCENARIO_ORDER:
        s = scenarios.get(name, {})
        if s.get("label"):
            lines.append(f"- **{name.capitalize()}**: {s['label']}")
    lines.append("")

    lines.append("**Exposure delta:** additional annual unhedged commodity cost vs base scenario.")
    lines.append("Positive = more expensive under this scenario; negative = cheaper.")
    lines.append("")

    lines.append("**Renewal flags under each scenario:**")
    for name in _SCENARIO_ORDER:
        s = scenarios.get(name, {})
        flags = s.get("renewal_flags", [])
        if not flags:
            continue
        lines.append(f"\n*{name.capitalize()} scenario:*")
        lines.append("| Customer | Segment | Days | Proposed Rate (£/MWh) | Current Rate (£/MWh) |")
        lines.append("|----------|---------|------|----------------------|----------------------|")
        for f in flags:
            lines.append(
                f"| {f['cid']} | {f.get('segment','?')} | {f.get('days_to_renewal','?')} "
                f"| {f.get('proposed_rate_gbp_per_mwh','?')} | {f.get('current_rate_gbp_per_mwh','?')} |"
            )
    lines.append("")
    return "\n".join(lines)
...
def generate_annual_report(data: dict) -> str:
    ...
    sections.append(_section_scenario_sensitivity(data))           # Phase PZ
