...
| 9 | **Shared-primitive census** (SP5, `docs/design/SHARED_PRIMITIVE_ENSURING_ACTIVITY_DISCOVER.md` §5) | `docs/observability/shared_primitive_census.json` — generated by `background/shared_primitive_census.py`, **never hand-edited**; the generator is its only writer (except the 5.4 review stamp, written only by `record_standing_review`) | **[BUILT + WIRED 2026-08-03 — `_REGISTERS["shared_primitive_census"]`.]** (a) `clone_count > clone_ceiling` (drift past the ratchet's recorded ceiling; this reader only reports against the ceiling, never raises it); **or** (b) a `shared_primitive_inventory` entry with `exists: true` and `migrated_count < caller_count` (a canonical primitive exists but a caller still runs the duplicate — an unfinished migration); **or** (c) a `quantity_registry_coverage` quantity with no declared owning module; **or** (d) `register_count` grew since the last recorded run with no `migration_note` in the same run; **or** (e) **the standing structural review itself (§5.3/§5.4)** — no `last_standing_review` stamp, a stamp older than the retro cadence's own staleness threshold, a malformed stamp, or a stamp whose verdict is `needs_work`. Fail-safe: missing/malformed/stale census ⇒ ONE `unreadable`/`stale` OPEN row, never a silent empty. |
