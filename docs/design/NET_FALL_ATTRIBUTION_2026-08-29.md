...
#### 3.4a RESOLVED 2026-08-29: the selector landed in `4d0ed7b7d`

The refusal above is left standing rather than edited away, because a refusal that names its reason
is how you find out whether the reason was the right one — and here it was: the condition it named
was "in no commit", and that condition is now false.

`saas.capital.solvency.mcr_accounts_on_supply`, its twelve controls
(`tests/company/risk/test_mcr_account_population.py`), the census document, and the wiring in
`simulation/run_phase2b.py` that makes the desk *count* rather than be told, are all in
**`4d0ed7b7d`**. The 120-account domestic-on-supply basis is therefore quotable, and §3.1's table
stays on 24 only as the record of what was shipped at the time it was written.

**The run current after the landing does not carry the same net assets as the run §3.3 was measured
on, and the difference is not this repair.** At `run_output_latest.json` as regenerated 17:28:

| | §3.3's run | the run now live |
|---|---:|---:|
| net assets | £331,361.47 | £362,561.47 |
| accounts (named basis) | 120 | **120** |
| MCR at £130 | £15,600 | £15,600 |
| free equity | £315,761.47 | **£346,961.47** |

The account count is the same on both, which is the point: the population is now a *named selection*
applied to whichever settled record the run produced, so it moves with the book rather than with the
roster. **Do not read £346,961.47 as this repair improving anything** — the £31,200 of extra net
assets comes from the run, not from the selector, and attributing it here would be exactly the
"more than one thing changed" error this document exists to avoid. What this repair did to free
equity is unchanged and is the only number attributable to it: **−£12,480**.
...
3. ~~**`mcr_accounts_on_supply` is unlanded.** Every MCR figure the site publishes is on the 24-leg
   basis until it lands.~~ **DONE 2026-08-29, `4d0ed7b7d`** — selector, twelve controls, census and
   the `run_phase2b` wiring, in one commit. See §3.4a. The two places the same defect can re-enter
   are named there and are not fixed: `company/finance/treasury.mcr_headroom` and
   `saas/capital/solvency.compute_solvency_signal` both still take a `customer_count` from their
   caller and neither names it.
