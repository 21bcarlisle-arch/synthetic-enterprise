...
| Banking & Payment Rails (Bacs/bank/acquirer cash arrival) | Bacs Direct Debit cycle (AUDDIS mandate setup, ARUDD failure returns w/ reason codes, ADDACS amendments), card acquirer settlement | Not yet built — registered 2026-07-11 (`docs/design/THE_VALUE_CYCLE_FRAMING.md` M2, maturity_map.yaml `W5_1_banking_payment_rails`) as a NEW W-lane component, built adapter-SHAPED from the start (not retrofitted later) since it's explicitly "a limb of the spine, not an adjacent system" — cash lands when the rails say, making the banked clock (D2_three_clocks) honest | A typed `PaymentRailsMessage`-style adapter mirroring the real Bacs cycle timing/failure-code shape from day one; full typed/versioned HARDENING of this seam (schema versioning, the reference-flow-conversion treatment given to the other rows) is Epoch-3 work, same as the rest of this table |
