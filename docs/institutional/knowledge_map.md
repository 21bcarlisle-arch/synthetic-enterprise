...
| Switching rates | **SETTLED 2026-08-30, on a declared pair.** External changes of supplier on a GB domestic ELECTRICITY meter point, over ALL domestic electricity accounts (~28m, incl. mid-fixed-term): 2016 17.0–17.6%, 2017 13.5–14.0%, 2018 19.5–20.0%, 2019 20.7–21.3%, 2020 22.5–23.0%, 2021 17.9–18.4%, 2022 2.9–4.3%, 2023 8.9–12.5%, 2024 12.5–16.1%, 2025 14.3–17.9%. Commons: `docs/domain_artefact_library/regulatory/gb_domestic_switching_rate.json`. Working: `docs/market_research/gb_switching_rate_denominators.md`. **This series was in the repo the whole time** (`churn_price_elasticity.md` §1, live-adjudicated in `f5_simulated_competitor_field.md` §9 in July) while this row read "—" and `market_report._UK_SWITCHING_RATE_PCT` shipped a different series that was outside the band in 9 of 10 years. | H | ElectraLink's headline series is not reconciled to a fuel (1.8× ambiguity, recorded not averaged); no per-supplier loss rates are published | **This cell no longer carries a measurement, and that is the repair.** It read *"the measured gap is 3.15× and the level move is not yet made"* until 2026-09-03 — a figure the **world's departure LEVEL** row below had already marked SUPERSEDED and said in terms *"must not be re-quoted as current"*, and a level move that had landed twice. One file, two rows, one forbidding what the other was doing. A reader sent here before touching the world's churn would have gone to raise a level that had already moved. **This row is the BAND: published evidence, settled 2026-08-30, and it does not move.** The world's distance from the band is a measurement that moves every capture, it has exactly one home, and that home is the row below — go there for the live state, the per-year margins and what is still owed |
...
| Customer lifetime / churn | **The aggregate identity answers the residential question and it was never written down.** Total switches = Σ suppliers' external losses; total accounts = Σ their books. So the account-weighted mean supplier loss rate IS the market switching rate exactly — 12.5–16.1% in 2024. Switching concentrates in engaged households, which makes suppliers differ from EACH OTHER; it does not lower the mean. The market rate is therefore the EXPECTATION for a book of average engagement, **not an upper bound on one** — a misreading that licenses aiming low, and one this project made in writing on 2026-08-30 and corrected the same day. | H (the identity) / — (per-type split) | No published per-supplier or per-customer-type loss rates; SME churn still uncalibrated | What makes a particular book differ from the market mean — that is the modelling question, and it is not a discrepancy to close by tuning |
