...
| **The world's departure LEVEL** | Measured 2026-08-30 against the published band above: **3.15× short** per renewal (4.93% vs a 2017–24 published mean of 15.5%), ~4.3× short per active electricity account (3.6%). Two stacked causes, both new: (1) `market_switching_propensity._savings_to_rate` says it is "calibrated from the DESNZ switching series" and runs at **half** it, missing 2020 by 2.9× while getting 2022 right — so it is wrong in shape as well as level; (2) `market_switching_multiplier` then **divides that absolute rate by its own 2024 value**, so whatever level the curve carries never reaches the run at all. The world took the SHAPE of the published series and normalised its LEVEL away. Instrument: `tools/measure_departure_level.py`. | H (the gap) | The level move is NOT yet made — this row records the measurement, not a repair | Move it, on the pre-registered prediction in `gb_switching_rate_denominators.md` §8 |
