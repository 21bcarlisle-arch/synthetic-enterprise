...
| **The world's departure LEVEL** | **The move was made, and as of 2026-09-03 the residual gap is small and LOW, not high.** `simulation/departure_level_anchor.py` landed a per-year level anchor; `tools/measure_departure_level.py::DEFAULT_TABLE` was repointed the same day (`5554c2910`) at `docs/reports/c4_whole_book_departure_factors.json` — the only capture on disk whose two halves are BOTH committed and which ran under the live anchor block. On it the whole book is **in band at 2024, high at 2020 by +1.62pp, and low in the other six years by −0.22 to −2.41pp**; mean 14.55% against a published midpoint of 15.50%. **Read the margin, never the verdict:** 2017 (−2.41pp) and 2019 (−0.22pp) are not the same claim. **What the instrument said until 2026-09-03, and why it was wrong:** on `c2_departure_factors.json` it reported **high in 8 of 8** and a mean of 22.35% — because c2's SVT half is in **no commit** and its 2022 SVT rows ran under the reference-year borrow (3.053619) that `NO_LEVEL_CORRECTION` replaced, while its 148 renewal rows all matched the live block. **The two halves of one capture disagreed, so a control reading either half alone would have passed.** That one stale year carried the whole apparent excess; strip it and c2's own remaining spread is +0.26 to +1.09pp. Two refusals now make the class fail rather than read: a capture must have run under the live anchors, and both its halves must be tracked. **SUPERSEDED, kept for the audit trail:** this row read "**3.15× short** per renewal (4.93% vs a 2017–24 published mean of 15.5%), ~4.3× short per active electricity account" from 2026-08-30, with two stacked causes — (1) `_savings_to_rate` running at half the DESNZ series, (2) `market_switching_multiplier` dividing that absolute rate by its own 2024 value so the level never reached the run. Both were repaired; the 3.15× is a pre-anchor figure and must not be re-quoted as current. | M — the direction is corroborated twice (this row's own 2026-08-30 low reading, and c4), the magnitude by ONE capture only: 156 renewal and 1,373 SVT decisions, a single draw | **The re-fit has NOT been run.** Six years still sit below their band and one above, so the instrument is now trustworthy and the world is not yet correct. c4 is one draw and the fit's own iteration is capture → fit → capture | Re-fit **upward** on the pre-registration in `docs/staging/SEAT_PREREGISTRATION_WHETHER_REPOINTING_THE_INSTRUMENT_AT_THE_COMMITTED_CAPTURE_HOLDS_2026-09-03.md`, which keeps `gb_switching_rate_denominators.md` §8 prediction 3 **uninverted** because the correcting move raises churn and makes the book harder to hold |
