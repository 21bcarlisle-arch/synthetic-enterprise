...
| **The world's departure LEVEL** | **Re-fitted 2026-09-03, and the world moved toward the record without reaching it.** The anchor was solved on `c4_whole_book_departure_factors.json` (156 renewal / 1,373 SVT decisions, both halves committed, run under the previous block) and the world re-captured under the result as `c5_refitted_departure_factors.json` — anchor and capture in ONE commit, because a re-fit landed without its capture makes every capture on disk stale and wedges every lane. **Six anchors rose and 2020 fell**, per year as pre-registered. On a ONE-VARIABLE pair (the control arm re-run today with nothing injected came out byte-identical to `c4` in both halves): mean distance OUTSIDE the published band **0.875pp → 0.425pp**, worst year **2.40pp → 1.30pp**, years in band **1 of 8 → 2 of 8**. **Six years are still outside**, so this is one pass of capture → fit → capture and not an arrival; the whole-book strict xfail stays on. It moved AGAINST the company, which is the direction a baseline change must be able to go: net margin **£126,488 → £120,933 (−4.4%)**, gross **£401,580 → £386,196 (−3.8%)**, no headline improved, so §8 prediction 3 did not fire. **What the instrument said until 2026-09-03, and why it was wrong:** on `c2_departure_factors.json` it reported **high in 8 of 8** and a mean of 22.35% — c2's SVT half is in **no commit** and its 2022 SVT rows ran under the reference-year borrow (3.053619) that `NO_LEVEL_CORRECTION` replaced, while its 148 renewal rows all matched. **The two halves of one capture disagreed, so a control reading either half alone would have passed.** Two refusals now make that class fail: a capture must have run under the live anchors and both halves must be tracked. **SUPERSEDED, kept for the audit trail:** this row read "**3.15× short** per renewal" from 2026-08-30, caused by `_savings_to_rate` running at half the DESNZ series and `market_switching_multiplier` dividing the absolute rate by its own 2024 value. Both repaired; the 3.15× is a pre-anchor figure and must not be re-quoted as current. | M — the DIRECTION is corroborated three ways (the 2026-08-30 low reading, c4, and the one-variable pair); the MAGNITUDE rests on single draws of 139–156 renewal decisions | **Six of eight years are still out of band.** The fit is exact on the population it was solved against and approximate on the next, so a further pass is owed. 2022 remains unfitted and unfittable from this capture family: zero renewal decisions, and the anchor has no path into `svt_inertia` | Run the next capture → fit pass and see whether the residual closes or plateaus; if it plateaus, the remaining gap is a mechanism question and not a calibration one. Grading in `docs/staging/records/SEAT_PREREGISTRATION_WHETHER_REPOINTING_THE_INSTRUMENT_AT_THE_COMMITTED_CAPTURE_HOLDS_2026-09-03.md` |
