...
|---|------|-------|--------|------------|
| 1 | ~~Persist structured Phase 2b/4c run output (`extract_report_data()`'s dict) to JSON at the end of every full simulation run~~ — **Closed 2026-06-14**: `annual_report.py` saves to `docs/reports/run_output_latest.json` via `--save-json`; subsequent regenerations load from cache. Cache versioned with git commit hash (item 13 also closed). | High | Low | none |
| 2 | ~~Integrate Phase 4b outputs (CLV, churn risk score, enterprise value, cost-to-serve, home-move win rate) into the combined run output~~ — **Closed 2026-06-15 (Phase 5b)**: `run_phase4c_on_phase2b.py` now runs both Phase 2b settlement and all 4b customer-value builders (`cost_to_serve`, `churn_model`, `home_move_win_rate`, `enterprise_value`) in one pass. Annual report includes enterprise value, CLV, cost-to-serve, churn risk per customer. | High | Medium | 1 |
| 3 | ~~Persist per-wake-up VaR ratio (current vs stressed floor)~~ — **Closed 2026-06-15**: `committee_wake_ups` entries already include `portfolio_var_current_gbp`/`portfolio_var_stressed_gbp`; `extract_report_data()` computes the average `var_ratio` per year and the report renders VaR current/stressed/ratio in each wake-up line. | Medium | Low | 1 |
| 4 | ~~Build a chronological portfolio treasury time series with drawdown-event detection (>10% peak-to-trough)~~ — **Closed 2026-06-15**: `extract_report_data()` builds a per-year treasury series sorted by settlement date/period; `_drawdown_events()` detects peak-to-trough events ≥10%; the Portfolio Health section in each year renders them (or "none" when clean). | Medium | Medium | 1, 3 |
| 5 | ~~Wire `saas/cost_to_serve.py` into the per-customer net margin calculation~~ — **Closed 2026-06-15 (Phase 5b)**: `extract_report_data()` populates `per_customer_lifetime[cid]["cost_to_serve_gbp"]` and `net_margin_after_cost_to_serve_gbp`; the Pricing & Margin section renders cost-to-serve and net-after-cts per customer (with a `**net-negative**` flag when applicable), plus portfolio total in the executive summary. | High | Medium | 2 |
...
| 7 | ~~Define a churn-risk threshold and surface "# customers above threshold" per year~~ — **Superseded by Phase 6b (2026-06-16)**: Phase 6b implements actual churn events (deterministic roll at each renewal point). "# customers who churned" is now a real count, not a probability threshold question — the section in the annual report shows the event table directly. | Medium | Medium | 2, design decision |
...
| 10 | ~~Hedge-effectiveness analysis section using the newly-exposed `hedge_evolution` (`actual_net` vs `naked_net` per term) — how much did hedging decisions add or cost relative to staying unhedged~~ — **Closed 2026-06-15**: `extract_report_data()` computes `hedge_effectiveness` (actual vs naked per customer, best/worst decision) per year; `generate_annual_report()` renders it in the whole-run summary and in each year section. | Medium | Medium | none (data exists) |
...
| 12 | ~~Dedicated administration/insolvency incident section~~ — **Closed 2026-06-16**: `_administration_section()` added to `generate_annual_report()`. Renders "None — business survived" when no event; expands to a full incident record with trigger date, customer, commodity, and treasury balance if ever triggered. | Low | Low | none |
...
| 14 | ~~`make report` as a scheduled/automated step~~ — **Closed 2026-06-14**: `make run` invokes `annual_report.py --save-json ...` which generates the JSON and the markdown in a single step. `make run-fast` added for development iteration. | Low | Low | 1 |
| 15 | ~~NTFY digest of the executive summary whenever the annual report is regenerated~~ — **Closed 2026-06-16**: `_send_run_complete_ntfy()` added to `annual_report.py`; fires after a fresh full run (not fast mode, not truncated) with net margin, treasury range, churn summary, and committee wake-up count. | Low | Low | 1, 14 |
...
| 18 | **EFFECTIVE_EAC_KWH calibration** (`simulation/run_phase2b.py`): declared EAC values for PC1 residential customers (C1=2800, C2=3500, C3=3200, C4=5500) do not match actual settlement consumption. PC1 base shape is fixed ~3900 kWh/year regardless of declared EAC; EV charging adds 8 kWh/night × 365 = 2920 kWh/year for EV customers. C2/C4 (EV) actually consume ~6820 kWh/year. Phase 23a `demand_estimation_log` shows the company correctly estimating actual billing (~7132 kWh for C2) but `error_pct` compares against the wrong oracle — creating misleadingly large "errors" of 100%+. **Fix**: update EFFECTIVE_EAC_KWH from actual settlement records; update `true_eac_kwh` in demand_estimation_log to use mean annual settled consumption, not pricing EAC. Changing EFFECTIVE_EAC_KWH alters pricing/hedging for C2/C4 — requires full re-run and is a deliberate phase. Discovered 2026-06-22 via Phase 23a analysis. | High | Medium | Phase 23a complete |
| 19 | **Solar generation not wired** (`simulation/demand_model.py` + `simulation/run_phase2b.py`): `build_demand_shape()` has a complete solar generation code path (subtracts irradiance-derived kWh from shape) but `_weather_adjusted_shape_fn()` never passes `irradiance_w_m2_periods`, so `assets.solar = True` in C4's property record has zero effect on electricity consumption. C4 behaves identically to a non-solar customer (~6820 kWh/year instead of expected ~4800 kWh with solar). **Fix**: wire half-hourly irradiance from `sim.weather_engine` into the shape function call, matching the pattern already used for temperature. Discovered 2026-06-22. | Medium | Medium | weather engine irradiance data available |
