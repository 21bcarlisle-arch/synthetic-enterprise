...
| 19 | ~~**Solar generation wired in Phase 25a**~~ — **Closed 2026-06-22**: `_weather_adjusted_shape_fn()` in `run_phase2b.py` does pass `cloud_cover_means` and `latitude_deg` for customers with `assets.solar=True` (C4). Verified: 2021-07-01 day, C4 demand 17.48 kWh without irradiance → 11.97 kWh with irradiance = 5.51 kWh solar generation (cloud cover 89%). Winter: 2.48 kWh generated. Solar reduction is real and working. Item 19 was incorrectly written as an open gap. | Medium | Medium | ~~weather engine irradiance data available~~ DONE |
