...
- **Phase OK: Energy Company Obligation (ECO) Observatory** -- UK suppliers with 150k+ domestic customers must fund ECO insulation/heating upgrades. eco_obligation.py exists but is not wired. Same threshold as WHD -- board section should confirm exemption and state the per-customer obligation rate if we scaled.
