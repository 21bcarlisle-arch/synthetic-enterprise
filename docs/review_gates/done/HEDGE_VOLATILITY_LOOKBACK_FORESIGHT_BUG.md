# REVIEW GATE (Tier 1 — one-way door): hedge-decision volatility lookback uses future data — CLOSED

**CLOSED 2026-07-10 (director NTFY reply "1. a", to "publish corrected figures now, drop the old
naked-hedging claim from the site" vs "hold until I've read the full writeup"):** sim-runner hold
removed (`docs/review_gates/.sim_runner_hold` deleted), daemon resumed normal operation. The
"Regime-Change Blindness" discovery card (`site/project/index.html`) and the hedge-fraction chart
caption (`site/supplier/index.html`) both rewritten to honestly reflect the re-derivation: the old
"naked hedging during calm markets" claim does not hold in the current build; population hedge
cover stays 0.80–0.90 throughout, and the only 0% entries trace to one structurally-pass-through
gas account, not a behavioural pattern. Framed as a caught-and-corrected finding (an earlier,
since-superseded architecture), not silently erased — matches this project's own "misses included"
public-track-record identity. Gate stays closed; next full sim-runner cycle publishes the
corrected canonical figures naturally.

**Status (historical):** DIRECTOR-AUTHORIZED IN-CONSOLE (Option A/B hybrid), 2026-07-10. Exact words: *"Hedge
