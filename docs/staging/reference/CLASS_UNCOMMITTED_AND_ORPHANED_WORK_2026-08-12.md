...
- `SEAT_FINDING_ONE_CLAIM_ID_TWO_SEATS_TWO_FORKED_TREES_AND_THE_COMPLETE_ANSWER_IS_STRANDED_ON_THE_SIDE_THAT_CANNOT_LAND_2026-09-11.md` — BLOCKING
...
- `SEAT_FINDING_ONE_CLAIM_ID_TWO_SEATS_TWO_FORKED_TREES_AND_THE_COMPLETE_ANSWER_IS_STRANDED_ON_THE_SIDE_THAT_CANNOT_LAND_2026-09-11.md`
...
- `SEAT_FINDING_THE_SHARED_TREE_HOLDS_AN_ELEVEN_DAY_BACKLOG_OF_UNLANDED_SOURCE_AND_IT_IS_WHAT_HOLDS_THE_FAST_FORWARD_OPEN_2026-09-10.md`
...
- `SEAT_RESULT_THE_STALE_KNOWLEDGE_MAP_IS_DISCARDED_AND_THE_DRAWS_OWN_TAG_LICENSED_THE_DOOR_THAT_WOULD_HAVE_LANDED_THE_REVERT_2026-09-23.md` — lane `A_strategy_governance`
- `SEAT_RESULT_THE_UNCOMMITTED_STAGING_DISPOSITIONS_WERE_27_MOVES_NOT_19_AND_THE_ITEMS_OWN_DOOR_WOULD_HAVE_UNARCHIVED_107_2026-09-23.md` — lane `A_strategy_governance`
...
**Taken:** 2026-09-01
**At:** 20 instances
**Because:** COVERED LATE AND INCOMPLETELY, which is why it is still accruing four instances a week.
The closing mechanism is *check the past-tense record against the commits*, and four controls of
that exact shape now exist:
`tests/architecture/test_no_committed_store_claims_an_unlanded_falsifier.py` (2026-08-18),
`tests/architecture/test_no_committed_discharge_cites_an_unlanded_falsifier.py` (2026-08-18),
`tests/architecture/test_no_committed_store_claims_an_unlanded_symbol.py` (2026-08-19) and
`tests/design/test_a_landed_claim_names_an_artefact_that_is_in_a_commit.py` (written 2026-09-01).

They arrived after most of the damage. Of the 20 instances, 11 predate 2026-08-18 and none of them
could have been caught. The four they WOULD have caught are all after it and all filed anyway,
because each control is scoped to one record type: *three committed discharges name falsifiers
that are in no commit* (2026-08-18) is the discharge control's own founding case; *three
consecutive passes recorded a landing that is in no commit* (2026-08-19) and *the map count was
bumped past a record that is in no commit* (2026-08-29) are the store controls' shape at a
different address; *an atom's cited evidence is never checked against any commit* (2026-08-18)
names the general form and is still open.

**The gap is that there are four controls and one property.** Each names its own record type, so a
fifth record type is uncovered by construction and nothing says so — which is how this class kept
accruing while its closure was landing. Consolidating those four into one property-keyed control is
the next work here. The twentieth instance is this morning's and it is the fifth record type exactly: a test file
rewritten ahead of an API that was never written took 413 architecture controls out of collection
for nine and a half hours, and none of the four controls is about a test file's imports. The
one-leg control for it landed with this disposition
(`tests/architecture/test_a_test_module_imports_a_name_that_exists.py`), which makes five narrow
controls where the argument above says there should be one.
