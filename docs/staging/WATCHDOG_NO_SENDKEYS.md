...
CONFIRMED LIVE BY RICH AT THE TERMINAL: bash -lc 'which claude' returns EMPTY. claude is installed via nvm, and nvm only initialises in INTERACTIVE shells (.bashrc guards exit for non-interactive). So bash -l is NOT sufficient -- this is why every watchdog launch attempt has failed. The absolute path must be resolved via:
  ls ~/.nvm/versions/node/*/bin/claude     (direct)
  or bash -i -c 'which claude'             (interactive shell)
NEVER bash -lc, which is proven to return nothing on this machine.

THREE send-keys failure modes are now proven: (1) timing race, (2) PATH/nvm (claude not found in ANY non-interactive shell incl. login), (3) quote-swallowing. Patching send-keys again is not acceptable. REMOVE IT.

THE REQUIRED DESIGN -- no keystroke injection anywhere:
1. Resolve the absolute claude binary path ONCE via ls ~/.nvm/versions/node/*/bin/claude, store it in the watchdog config. Re-resolve on failure (node version upgrades move the path).
2. Launch the session with the instruction AS AN ARGUMENT, no send-keys:
   tmux new-session -d -s claude "<ABS_NVM_PATH>/claude --dangerously-skip-permissions \"Read docs/RESUME_INSTRUCTION.md and follow it\""
...
VERIFY BEFORE CLAIMING DONE (per Rule 2 -- violated twice on this exact component):
