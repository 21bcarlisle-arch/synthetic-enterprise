...
## Deliverables 1–3 → minted atoms (lane H_harness, close_to_learn, level 0→3)

**One console act needed to make them draw (surfaced as [ACT]).** H_harness is off every open product front (SIM_ACTORS=W1/W2, SUPPLIER=D/B/E/C/F, EPOCH2_CAMPAIGN=G) and fronts enforcement is ON — `filter_build_candidates` returns EMPTY for HX1–3 (verified live). So `blocked_on: null` would be a *false* drawable state; corrected to name the gate. The ruling COMMANDS this work, but the executing act is a per-atom **BUILD_OPEN in `gate_authorizations.jsonl`** (or an H-lane FRONT_OPEN), which is director-console-only (R16 — I cannot self-authorize; and decision 2a itself demotes general harness BUILD until the counter is met, so these three are the *bootstrapping exception* you must open). One line — `BUILD_OPEN{atoms:[HX1_exit_criterion_counter_mechanise, HX2_stall_set_coverage_verdict, HX3_counter_published_and_derivable]}` — and the next tick draws them in dependency order.
