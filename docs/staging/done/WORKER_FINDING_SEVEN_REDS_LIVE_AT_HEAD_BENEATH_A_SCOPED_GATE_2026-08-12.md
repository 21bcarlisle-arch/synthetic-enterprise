...
**Discharged:** `tests/tools/test_head_green_census.py::test_a_new_red_is_the_alarm_and_names_itself`, `tests/tools/test_head_green_census.py::test_the_marker_expression_matches_the_publish_gate`, `tests/tools/test_head_green_census.py::test_a_run_that_proved_nothing_is_UNPROVEN_not_GREEN`, `tools/head_green_census.py`, `background/head-green-census.timer`, `tests/tools/test_measure_publish_gate_subject_cost.py`, `tests/tools/test_pre_commit_test_gate.py` — this finding had TWO subjects and both are evidenced, because discharging the instances while the blind spot stayed open would be exactly the laundering clause 2 forbids. (1) The seven reds: all seven RE-RUN at this HEAD in one invocation, 129 passed, including the three peak-sampler cases and the env-scrub guard that was the live hazard. (2) The blind spot — that no routine gate was SHAPED to see a red at HEAD — is the falsifiable half, so it is cited first: the census's subject is DERIVED from the publish gate's own marker expression rather than restated, a run that proved nothing reads UNPROVEN and never GREEN, and a new red names itself. R2 checked, not assumed: the timer unit named above is loaded and active with a next elapse, so the mechanism is RUNNING and not merely committed.
