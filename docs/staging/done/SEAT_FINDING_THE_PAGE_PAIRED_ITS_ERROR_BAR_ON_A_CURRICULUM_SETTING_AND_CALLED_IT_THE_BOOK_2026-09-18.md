...
> **CLOSED 2026-09-19 on the condition this document set itself** — "when that floor lands and is
> admitted on the book rather than waved through on a curriculum setting". It has and it is:
> `NOISE_FLOOR_PATH` now names the twelve at `18327d977`, `_floor_admission` returns
> `admitted: True` on the declared half AND the realised one, `_staleness_caveat` returns `None`,
> and `contrast_bounds.available` went `false -> true`. Both gating predictions of the
> pre-registration held. Severity dropped from BLOCKING here rather than in a second document, so
> the lane's books and this claim cannot disagree. Result and the measurements:
> `SEAT_RESULT_THE_DRAWN_REPAIR_WAS_ALREADY_LANDED_AND_THE_FLOOR_THAT_UNBLOCKS_THE_PAGE_MET_ITS_PRE_REGISTERED_RULE_2026-09-19.md`.
