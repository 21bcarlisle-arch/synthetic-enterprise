...
<!-- BLOCK_RELEASE: propose_then_proceed -- was 'director_level_up', an act abolished 2026-07-29 and swept 2026-08-03: propose, record, act. Original note: BUILD half (b)(c)(d) LANDED 2026-07-28 (reader background/gap_register_scan.py with corrected keys + gap_register detector level in authorized_set_enumeration() + R15 both-ways incl the 4 mandated mutations, 10 tests green; 292 supervisor/deadman/worker-tick tests green); only the level_current 0->3 move remains, director-reserved (R16). -->
<!-- STATUS 2026-07-28 (worker tick): BUILD half DONE. background/gap_register_scan.py enumerates the OPEN residue across all 8 registers from PRIMARY state (independent reader, imports nothing from supervisor -- invariant 1/LAW-C); registers 1 (text-heuristic, not the non-existent measured_bound field) + 6 (state-keyed, prefix-agnostic, not audit:*) are the two corrected FAIL-OPEN keys, each proven by a mutation that FIRES. gap_register wired as a detector level (Y => rest illegitimate) with a .gap_register_level_disabled kill flag. Fail-safe toward work on any read error (invariant 2). LEVEL MOVE 0->3 stays blocked_on director_level_up (R16 -- agent proposes level, never moves the cell). -->
