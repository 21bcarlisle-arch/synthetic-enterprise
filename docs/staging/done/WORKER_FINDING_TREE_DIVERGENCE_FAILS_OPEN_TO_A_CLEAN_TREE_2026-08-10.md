...
**Discharged:** `tests/background/test_tree_divergence.py::test_changed_paths_says_unknown_not_nothing_when_git_cannot_answer`, `tests/background/test_tree_divergence.py::test_an_unavailable_measure_omits_the_counts_so_no_reader_can_read_it_as_zero`, `tests/background/test_tree_divergence.py::test_breaches_names_the_unavailability_as_its_own_breach`, `tests/background/test_tree_divergence.py::test_a_git_timeout_is_unavailable_too_not_clean`, `tests/background/test_tree_divergence.py::test_the_publish_path_NAMES_an_unavailable_measure_instead_of_swallowing_it`, `background/tree_divergence.py`, `background/process_run_complete.py` — repaired in the shape this document specified, all three numbered points. The git read now returns None rather than the empty list, carrying the reason with it; the measure OMITS the counts rather than zeroing them, so a reader that has never heard of the flag gets a loud KeyError instead of a quiet clean bill of health; and an unmeasurable tree is its own named breach, checked first, so the daily naming still fires saying the true thing. A timeout is included, which the document did not ask for but the same silence wears: it was escaping into the caller's blanket except. The caller was repaired too — it logged the counts before calling breaches, so the omission alone would have reinstated the identical silence one layer up, and that is the falsifier named last above. Verified by RUNNING the falsifiers at this HEAD, not by reading the commit: 21 passed, and four separate mutations (the original empty-list return, the dropped breach branch, zeroed counts, the unconditional caller log) each reddened their own named test with both files checksum-verified restored. It still cannot punish: the publish path helper returns None, so there is no value to branch on.
