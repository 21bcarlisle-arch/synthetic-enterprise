...
**Severity:** LATENT · **Lane:** H_harness · **Disposition:** BUILT 2026-08-17 (was QUEUED)

**Discharged:** `tests/background/test_publish_gate_wedge_draw.py::test_superseded_clause_fires_when_every_failure_predates_head`, `tests/background/test_publish_gate_wedge_draw.py::test_superseded_clause_is_silent_when_failures_are_at_head`, `tests/background/test_publish_gate_wedge_draw.py::test_superseded_clause_is_silent_on_a_mixed_hash_set`, `tests/background/test_publish_gate_wedge_draw.py::test_superseded_clause_is_silent_without_a_usable_hash`, `tests/background/test_publish_gate_wedge_draw.py::test_superseded_clause_is_silent_when_git_cannot_answer`, `tests/background/test_publish_gate_wedge_draw.py::test_in_flight_clause_fires_and_suspends_the_enumerate_instruction`, `tests/background/test_publish_gate_wedge_draw.py::test_in_flight_clause_is_silent_when_no_gate_is_running`, `tests/background/test_publish_gate_wedge_draw.py::test_live_gate_runs_parses_ps_and_ignores_grep_and_junk`, `tests/background/test_publish_gate_wedge_draw.py::test_live_gate_runs_is_empty_when_ps_is_unavailable`, `background/supervisor.py` — built exactly as this document specified, as TEXT ONLY: the two new clause helpers add sentences and neither can return None, so the draw still fires whenever there is no green at HEAD. What moved it from QUEUED to BUILT was a SECOND priority-zero tick lost to the same misread 8 minutes later (11:16 UTC), against this document's own predicted cost of one. Both directions proven per R15, and the silent direction is the one that matters: failures recorded AT HEAD, a MIXED hash set (the wedge outlived a commit), a missing or unknown hash, and git unable to answer ancestry all print nothing and leave the message identical. Verified against the real recorded state rather than a fixture — the four real failures at commit 1ffe5e219 under HEAD 39aff4a50 produce the superseded clause naming the log command that spans them, and the genuinely live PID 126285 produced the in-flight clause. 31 green in this file, 138 green across the wedge/publish-gate consumers, 2026-08-17.
