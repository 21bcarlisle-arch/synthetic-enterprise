# [DIRECTOR-CANON] — PITCH v7, VERBATIM. Adopt as purpose canon; reconcile against primary state. (2026-07-28)

**Type:** [DIRECTOR-CANON + reconciliation instruction]. Director-authored, supplied verbatim below. **Do not edit, condense or "correct" the director's text.** Adopt as `docs/design/PITCH_V7.md`, superseding v6.

## The layer division (this is the coherence ruling, applied to the pitch)

- **The pitch is canon for PURPOSE and ARGUMENT** — why this exists, what it claims, what it is for.
- **`THE_MODEL_ON_A_PAGE.md` is canon for STATE** — what exists now, what is planned.
- **The site derives from both.** Neither overrides the other; where they disagree that is a **finding for the director**, never an edit by whoever touched a file last.

## Reconciliation required (R9 — evidence, not narrative)

Reconcile the pitch line-by-line against **primary state**: the maturity map, the fidelity ledger, the simplifications and suppression registers, the gate-authorizations ledger, and the ratified rulings. For every claim describing what exists or what was found: **verified / stale / contradicted / not-checkable**, with its citation. Claims in the conditional about future states are exempt — the document is deliberately careful about tense; check that it stayed careful.

**Four the advisor already found. Find the rest — this list is not the audit.**

1. **§12, the payment blind-spot bullet CONTRADICTS a ratified ruling.** It cites as a virtue "a class of payment failures it structurally cannot observe." `f6238fd7f` (2026-07-25) **rejected** that: a real supplier detects a missed push payment by expected-collection reconciliation, so the honest residual is a **detection latency with a measured distribution**, not a structural blind spot. The pitch currently offers an overturned finding as evidence of its own honesty — the one place it cannot afford to be wrong. Propose the corrected wording; the director rules it.
2. **§12, the spike-tail bullet is STALE.** "Declared publicly as the blocking defect" — it was closed 2026-07-23. Propose replacement wording that keeps the honesty of the original (a real defect, self-found, published) without asserting a current block.
3. **§9 says "Two disciplines keep this honest" and then lists three.** The v7 usage-only ledger clause was appended without updating the count.
4. **The footer reads "Draft v5"** while the header reads v7.

## Standing consequence

The claim-status discipline the pitch itself declares now applies **to the pitch**: any figure or finding it cites must be derivable from primary state at publication time, and a citation that has since been overturned or closed is a **claim-status defect** — the same class as an aspiration written in the present tense on the site.

## Empowerment

**The reconciliation method is yours** — how you walk the claims, what you cite, how you batch the findings. If you judge some claims not-checkable by design (argument rather than fact), say so and classify them that way rather than forcing a verdict. If you find the advisor's four are wrong, argue with evidence.

## WORK THIS CREATES

1. Pitch v7 adopted to `docs/design/PITCH_V7.md`; v6 superseded.
2. A line-by-line reconciliation with per-claim verdicts and citations.
3. Proposed corrections for items 1–4 and anything else found, returned for director ruling — **do not self-edit the director's text.**
4. Any pitch/model-on-a-page disagreement surfaced as a finding.

Acceptance: every factual claim in the pitch carries a verdict traceable to primary state, and no cited finding is stale or overturned.

**Risk & proportionality:** canon adoption plus an analysis pass; no build, no self-editing of director text. Tag: **proceed.**
