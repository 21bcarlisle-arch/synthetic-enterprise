...
<!-- ACTIONED 2026-07-08: rewrote site/index.html (Home) and site/project/index.html
     (Project overview) per the canonical narrative points. Home: hero rewritten for
     end-state framing (point 1) with epistemic-boundary test folded in (point 4); new
     "Why This Is Different" section with two cards -- Proof in Public (point 2, linking
     to a new id="track-record" anchor added to site/method/index.html's Track Record
     section) and AI-native inversion (point 3, no vendor names, ties in mortality/
     point 5 via the crisis-survival link); Three Products reordered Method-first per
     point 7 (was Software/Data/Method). Project: About card rewritten with end-state +
     epistemic test + mortality framing, plus a paragraph naming the product ordering and
     linking to the track record; Roadmap section gets one new framing sentence (point 8,
     "path to the end state") above the unchanged checklist -- evidence sections (Key
     Discoveries, crisis survival, etc.) left as-is per the instruction, no new panels/
     charts/data surfaces added. No hardcoded stamps introduced -- all figures still
     data-driven via the existing dashboard.json/phases.json fetch.

     VERIFIED, not just written: node --check on extracted <script> blocks (all 3 touched
     files parse clean); tag-balance sanity check; then actually rendered in a real
     browser (headless Chrome via WSL/Windows interop -- --virtual-time-budget=5000 was
     the fix for an earlier session's screenshot timeout) and looked at the screenshots --
     Home and Project both render correctly with live data (net £1,535,308, EV £8,930,211,
     15,996 tests, 454 phases). The #track-record anchor screenshot came back blank (a
     headless-Chrome fragment-scroll capture quirk) but curl confirms the id="track-record"
     element exists in the served HTML at the right place -- not a real defect.

     NTFY sent noting deployed status. Nothing from the canonical points was deliberately
     dropped; AI-native inversion (point 3) is covered fully on Home only, not duplicated
     on Project, to keep Project's About card from becoming a second wall of the same text
     -- the DoD reads as "Home and Project together" cover all points for a first-time
     visitor, not each page individually repeating all six. -->
