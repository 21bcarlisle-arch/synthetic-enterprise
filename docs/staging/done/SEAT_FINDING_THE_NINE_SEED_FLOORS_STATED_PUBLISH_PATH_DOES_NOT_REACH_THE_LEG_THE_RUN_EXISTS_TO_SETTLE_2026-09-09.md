...
**Discharged:** 2026-09-16. `tests/tools/test_generate_value_arms_data.py::test_the_generator_reads_the_current_world_floor_from_its_own_constant`, `tests/tools/test_generate_value_arms_data.py::test_the_selection_leg_and_its_error_bar_are_published_together`, `tests/tools/test_generate_value_arms_data.py::test_the_real_page_prices_the_selection_legs_sign` — all three green in this tree. The defect was that CURRENT_WORLD_NOISE_FLOOR_PATH fed the selection leg a three-seed floor while the contrast_bounds block beside it already read the nine-seed one; the constant in `tools/generate_value_arms_data.py` now names the nine-seed floor, and the shipped `site/data/value_arms.json` reads current_world.selection_leg.floor_generated_at = 2026-09-09T15:17:31Z with current_world.bound.n = 9 against contrast_bounds.seeds = 9. Two blocks, one sample. The repair took the route this document argued for and NOT the one it warned against — the constant moved, the bytes at the 2026-09-08 floor artefact were not overwritten — and the constant's own comment records the code-tree difference between the two floors rather than waving it through. **What this does NOT discharge**, named rather than absorbed: the leg still states no direction at n=9, so the selection question the run existed to settle is still open. That is the atom's work, not this finding's.
