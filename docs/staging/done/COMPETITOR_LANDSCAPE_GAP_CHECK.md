...
<!-- ACTIONED 2026-07-08: docs/market_research/ESTATE_GAP_ANALYSIS.md written -- 10
     named capabilities cross-checked against real Poesys modules (grep/read-verified,
     not recalled): 7 COVERED, 1 already-named GAP (market messaging/DCC, confirmed
     unchanged), 1 OUT-OF-SCOPE (migration tooling, pre-real-customer), and 2 nuanced
     partial items (margin-leakage detection and pass-through-tariff price automation --
     the underlying capability exists via a different mechanism, but a genuinely narrower
     real-time/automation gap sits underneath it). Also found the existing
     SAAS_COVERAGE_MAP.md's own backlog item 1 (debt-sale/DCA placement) is now stale --
     Phase QS already closed it -- flagged, not fixed (desk analysis only). Registered
     the survey document's provenance in ASSUMPTIONS.md per the discipline instruction.
     Three ranked GAP items for the P-5 backlog. No implementation started. -->
