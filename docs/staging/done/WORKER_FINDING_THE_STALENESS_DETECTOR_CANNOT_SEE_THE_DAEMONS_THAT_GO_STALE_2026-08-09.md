...
## CLOSED 2026-08-09 — atom `PW1_staleness_is_code_actually_loaded`, L0→L2 (self-certified)

Both halves of "what closing it looks like" are built, plus the half this finding did not name (the
always-red problem, which the director raised separately as DECIDED #2).

**(a) Population — observed, never declared.** `_systemd_owned_sessions()` is **deleted**, not
deprecated. `observed_launched_by()` derives each daemon's launcher from unit-active + MainPID +
a `/proc/<pid>/cgroup` cross-check that can only REFUTE (an unreadable `/proc` must never shrink the
population — that was the same fail-open shape one layer down). `launcher_drift()` turns a wrong
manifest row into a loud `MISDECLARED_LAUNCHER` instead of a silent exclusion. Live population went
**5 → 12** sessions; `sim-runner` and `background-worker` are in it.

Point 3 of this finding — "close at the class: a control whose population is a declaration rather
than an observation" — is what the replacement test enforces. The old
`test_systemd_owned_sessions_are_only_the_migrated_ones` asserted a hand-written list against the
same hand-written manifest, i.e. a tautology (R15), which is exactly why it stayed green through
seven wrong rows. `test_manifest_launched_by_agrees_with_the_observed_world` compares the
declaration to the live box, so the next half-done cutover reds without anyone extending a list.

**(b) Signal — the modules this daemon loads.** `loaded_code_drift()` intersects each daemon's own
static import closure (`background/code_closure.py`, follows lazy in-function imports transitively)
with `git diff <boot_sha> → working tree`. Working tree, not HEAD, deliberately: the uncommitted
`sim_runner.py` argv change this finding closes with is exactly the case a HEAD diff would miss.
Live: 12 observed, **5 red, 7 green** on a moved HEAD — the signal now discriminates.

**R15, three mutations, each killing named tests:** declaration-filter-shrinks-population → 3;
signal-reverts-to-HEAD-comparison → 2; population-emptied → 4. Vacuity guard: a `vacuous` flag
(empty population while units are active is a FAILED check) plus a live test asserting a non-empty
daemon set. Unknown never reads as clean — `unstamped` / `closure-unknown` / `sha-unresolved` each
land in `unresolved` with their reason.

**Named replay (the exit criterion):** `sim-runner` against boot state `fa9a73c72` reads RED on
`background/sim_runner.py`, the file it was running an old copy of for ten hours. The old detector
answered "clean" for that same state, because sim-runner was not in its population at all.

**Still open, deliberately, and NOT part of this atom:** point 4 — the deploy step. `boot_sha`
remains REPORT-ONLY; nothing yet closes the loop from "stale detected" to "restarted" (G-D2). The
detector can now name the five stale daemons and the exact modules each is behind on; acting on that
is a separate atom.
