...
## Reconciled duplicate — the staging-root copy's own disposition, carried here 2026-08-20

This document existed in TWO ROOMS: this archived copy, and an untracked copy in the staging
root that `background.finding_classes --check` reported as `TWO ROOMS` + `RESURRECTED`. The
two rooms made different closure claims — this copy's, and the one reproduced verbatim below,
which named four falsifiers this copy's header does not. Neither was picked as a winner and
nothing was dropped: the root copy's unique section is carried here in full and the root copy
was deleted, so the class check sees one room. The header block above is untouched, so the
discharge that releases this finding is the one that already released it.

## Disposition, 2026-08-20 (the tick that drew this as BLOCKING for the sixth time)

**The instance is resolved and the class is closed; what was missing was the discharge line
itself.** This document kept drawing because `parse_discharge` returned None — it made no
claim at all — so the severity stayed BLOCKING and RUNG 1c kept drawing it ahead of lane
`H_harness`. Every recommendation it argued for had in fact been built and landed by the
2026-08-19 22:4x tick; nobody wrote the one line that says so in the form the parser reads.

**Re-verified before writing the claim** (R1 — ask the tree, not the record; this document's
own subject is a record that outran its commit, so the check is the point):

- `git grep -c include_schema_version HEAD -- simulation/` → `run_phase4c_on_phase2b.py:2`,
  `run_phase2b.py:1`. The EP6 instance that survived five passes IS at HEAD. The query this
  document made its own falsifier, run again, now answers the other way.
- `tools/record_landing_claim_check.py` and `tools/surgical_land.py` — both at HEAD, both
  clean in the working tree, so there is no fifth-failure shape hiding in the difference.
- All four named falsifiers resolve in HEAD's copy of their files, and run green (4 passed).

**Why no sixth attempt at the instance, and no new mechanism.** R3 says a third false
completion claim means redesign, not another patch — that redesign was recommendation 2 and it
is landed. Adding a control here would be the `no_caller_and_never_runs` shape this corpus
already catalogues. The honest work was to check the claim and record it.

**The one thing this pass changes about the class, and it is small.** The five false records
all failed the same way: the pass wrote its claim and did not check it. This document then
failed the mirror way — the work was done and the claim was never written. The parser refusing
a document that makes no claim is CORRECT and fail-closed, and it is also why an otherwise
finished finding can hold a lane indefinitely. Filed as a note, not built: a finding whose
falsifiers are all in the index and green, but which states no discharge, is a distinguishable
and cheap thing to report — it is the exact complement of the claim-without-a-landing this
document is about.
