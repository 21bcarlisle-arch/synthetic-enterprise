...
> **DISCHARGED 2026-09-18** (worker), and the reason it was deferred did not survive re-asking: the
> file is in **no ref at all** — not another lane's WIP — and it carries its own REUSE block, so
> both halves of the refusal above were false by the time anyone read it. Landed with a correction
> one line larger than the one owed here: the reporting call alone would have printed the census
> count beside kWh the generator produced from the bedroom count, so the profile is now drawn once
> and handed to `generate_premise_trace(behaviour=...)`. C1's explained electricity fell 44% and its
> hot water 45%. See
> `WORKER_RESULT_THE_EXPLAINER_THE_DIRECTORS_PHASE_ONE_TEST_RESTS_ON_WAS_IN_NO_REF_AND_DESCRIBED_A_HOME_THE_BOOK_NEVER_SETTLED_2026-09-18.md`.
