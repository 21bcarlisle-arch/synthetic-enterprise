...
**Discharged:** `tests/tools/test_site_reachability.py::test_the_LIVE_KNOWLEDGE_SECTION_is_reachable_from_the_front_door`, `tests/tools/test_pre_commit_gate_site_surface.py::test_the_selected_control_goes_RED_on_a_tree_with_an_orphan_page`, `tests/tools/test_site_freshness_stamps.py::test_no_header_stamp_claims_data_newer_than_its_newest_datum`, `tools/site_reachability.py` — all three items actioned in commit eb4a091bb and verified on the LIVE surface, not the repo (R11). Item 1: the cause was that the section had no index at all and each topic page's nav door pointed at itself, so nothing could link it; a section index now routes all nine pages and https://poesys.net/knowledge/ serves them. The ask behind it — a build-time control instead of the director's eyes — is tools/site_reachability.py, whose subject is DERIVED from sitemap.xml's own unenforced promise and from _redirects, plus a SITE surface trigger on the pre-commit gate, which previously skipped site/ entirely as pure data. Item 2 was NOT the false alarm it might have been: the header claimed 2026-07 over charts stamped 2025-06-07 while the upstream Elexon extract genuinely ends 2025-06-07, so the stamp was corrected to the data (never the reverse) and the class closed by a freshness control over all of site/data; the live feed now reads 2025-06. Item 3 shared item 1's cause and the nav is right-aligned on the pages that were not. R15 both ways including a live mutation: one unreachable page added to the real tree turns three controls red. Two reversible calls were made rather than asked — the seven stub pages are routed rather than hidden, and Knowledge sits on the secondary line rather than being promoted into the SITE_V5 canonical door set, which is the director's to widen; both were put to him by NTFY with a recommendation.
