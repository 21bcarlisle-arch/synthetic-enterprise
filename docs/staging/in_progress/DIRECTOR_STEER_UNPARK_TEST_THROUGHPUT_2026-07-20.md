...
> **Open sub-item:** un-park and resume the systematic throughput regime whose parked design lives at
> `in_progress/DIRECTOR_STEER_TEST_THROUGHPUT_2026-07-19.md` + `docs/design/TEST_THROUGHPUT_MEASUREMENT_AND_PROPOSAL.md`.
> Prioritise the population-scale-sensitive parts FIRST: (1) impact-based/predictive selection, (2) the
> stochastic/deterministic split — ahead of (3) cadence tiering and (4) the cheap structural wins.
> **Non-negotiables:** measure-before-restructure (extend the existing doc); R15 prove-no-safety-loss
> by mutation at each step; do NOT restructure gates + selection in one turn (own commits, revert path each).
> **Sequencing wall:** this regime must LAND before the reserved population-generator widening (the [ACT]
> above) so the larger population arrives into a validation regime that can afford it. Runs *together with*
> the value-frontier analysis but not blocked by it. Blast radius = every gate → incremental only.
