...
## Correction, appended 2026-08-31 by the next seat turn, beside the claim rather than over it

Two things above were wrong when written and are corrected here rather than edited out.

**1. The whole-book fit had NOT landed.** The paragraph immediately above says it had. It had not:
`tools/fit_year_level_anchor.py` at that commit contained no whole-book target and still refused on
`emission_refusal`'s minority clause. The measurement had been taken — in a scratch copy — and the
tool was never changed, so the numbers below the claim were real and the claim about where they came
from was not. **The whole-book fit is landed by the commit carrying this correction**, which is also
the commit that first lands the union the section above describes. This is the
"a finding can describe its own landing in the past tense while nothing was committed" shape, and it
was caught by reading `git status` before trusting the prose.

**2. Every number in the Result tables above used the WRONG combination within the account, and
the fitted anchors are superseded.** The tables sum an account's hazards (`Σp`); the landed
`departure_population.book_level_from_hazards` — and the whole of `departure_risks` — uses the
competing-risks form `1 − Π(1 − p)`. The two are reproducible against each other exactly: 2022's
SVT-only level is `Σp` = **12.8024%** (the figure printed above) and `1 − Π(1−p)` = **12.0889%** (the
landed figure, and the one the seat finding of the same day reports). One cause explains all of it —
every SVT floor above is ~5% too high and every fitted anchor ~12% too low.

The additive form is the defective one and not merely a different choice: it is unbounded above,
so an account facing eleven cap-period decisions can be assigned a departure probability over 1.

**Re-measured on the same capture through the landed instrument:**

| year | record % | SVT floor % (was) | fitted anchor (was) | committed anchor | direction |
|---|---|---|---|---|---|
| 2017 | 14.00 | 8.80 (9.27) | 4.547299 (4.027397) | 4.256902 | higher |
| 2018 | 20.00 | 10.66 (11.36) | 2.882178 (2.538637) | 3.345826 | lower |
| 2019 | 21.30 | 10.85 (11.60) | 4.803900 (4.348151) | 3.228064 | higher |
| 2020 | 23.00 | 9.30 (9.85) | 6.412007 (5.849491) | 4.425742 | higher |
| 2021 | 18.40 | 9.08 (9.62) | 4.488202 (4.042171) | 3.219914 | higher |
| 2023 | 12.50 | 11.69 (12.43) | 0.364038 (0.029981) | 2.091517 | lower |
| 2024 | 16.10 | 8.66 (9.12) | 3.053619 (2.800513) | 3.020806 | higher |

**Which predictions this changes: none of them, and that is worth saying explicitly rather than
leaving a reader to check.** P1 confirms on a wider margin (2022's floor is 12.09% against a band
topping at 4.30%, so the year is unreachable by more than the error). P4 is a decision count and is
untouched. P2 stays refuted with the same split — five years fit higher than the committed constant
and two lower. P3 was already computed on the landed form by the seat finding and is unchanged at
exactly 5 of 8. The correction moves every number and no verdict, which is the most that can be said
for it.

**2016 and 2025 now refuse for a stated reason rather than being dropped by hand.** The block above
omits them with the note *"absent on purpose"*; the landed fit refuses them against
`measure_departure_level.COMPARISON_YEARS`, the window this repository had already declared and
justified (2016 holds one renewal decision, 2025 is partial). Reusing that window rather than
minting a threshold is why the refusal carries a reason a reader can check. Without it the fit
emitted **2016: 15.988769** — a constant to six decimals solved off a single renewal decision, which
nothing in the block it landed in would have flagged.