...
fired **1 times without its state changing**, over **3.1h**. Under the
...
What the seat said it was doing: Two commands, in this order, landing after EACH one rather than batching. FIRST `python3 -m tools.run_value_cycle_ab` with no `--end-year`, then edit the headline table in `docs/design/THE_VALUE_CYCLE
```

## What is known without diagnosing anything

- Signature: `seat-claim:run-both-instruments-at-full-window` — the alarm text with elapsed times, counters, hashes and timestamps
  normalised away, so this is the same CONDITION recurring, not the same string.
- First seen in this episode: 2026-08-27T05:51:03+00:00
