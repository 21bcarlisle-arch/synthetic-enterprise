...
# --- Structural bill-integrity invariants (atom F6_bill_integrity_structural) ---
#
# INVARIANT_LIBRARY_REDTEAM.md (2026-07-13) gaps 2-4: three load-bearing
# structural sanity controls any real biller has, absent here until now.
# Registered as the named/sourced CLASS rules (R10 -- closure is the class,
# not the instance); enforced by the same-named check_*() predicates below and
# wired into the pre-bill validation gate. Each is mutation-tested (R15): inject
# its exact named defect, the control MUST fire. All fail CLOSED on
...
BILL_FOOTS = StructuralInvariant(
    ...
    description=(
        ...
        "own line items -- commodity + non-commodity + standing charge + VAT -- "
        "to the penny. Named defect: a GBP 999,999 total printed on GBP 88 of "
        "line items, which passed because the total was never asserted to foot. "
        "Not-applicable only when no total is declared (nothing to disagree "
        "with the components); a present-but-unreadable total fails CLOSED."
    ),
...
# --- Structural bill-integrity predicates (atom F6_bill_integrity_structural) ---

# Penny-rounding headroom across up to four independently-rounded line items.
...
def is_credit_bill(bill: dict) -> bool:
    ...
    positive-indicator exemption to bill-sign sanity -- never a blanket
    negative pass. Shared by check_bill_line_items_non_negative() here and the
    pre-bill gate's subtotal check so the two can never diverge."""
...
def check_bill_foots(bill: dict) -> bool:
    ...
    the bill's own line items (commodity + non-commodity + standing + VAT) to
    within a penny-rounding tolerance.

    Not-applicable (True) only when NO total is declared -- there is then
    nothing to disagree with the components, and generate_billing_ledger
    recomputes the total from the same line items downstream. A PRESENT but
    non-numeric total fails CLOSED (R15: a total we cannot read cannot be
    reconciled). Sign-invariant, so it holds equally for a legitimate credit
    note (both total and components share sign)."""
...
def check_bill_line_items_non_negative(bill: dict) -> bool:
    """BILL_LINE_ITEMS_NON_NEGATIVE enforcement: every line item must be
    non-negative, EXCEPT for a legitimate credit note (is_credit_bill). A
    malformed (non-numeric) amount fails CLOSED. Uses the same penny tolerance
    so a -0.00x rounding artefact is not mistaken for a genuine negative
    charge."""
    if is_credit_bill(bill):
        return True
    for key in _LINE_ITEM_KEYS:
        try:
            val = float(bill.get(key, 0.0))
        except (TypeError, ValueError):
            return False  # fail closed
...
def check_bill_period_sane(bill: dict) -> bool:
    ...
    the previous silent clamp-to-1-day."""
