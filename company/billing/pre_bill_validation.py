...
def validate_bill(bill: dict) -> BillValidationResult:
    ...
    # F6_bill_integrity_structural (INVARIANT_LIBRARY_REDTEAM.md gaps 2-4): the
    # three structural sanity controls any real biller has, enforced as class
    # invariants in domain_invariants.py -- arithmetic footing, sign
    # non-negativity, and temporal sanity. Each HOLDS the bill (Tier-1) rather
    # than issuing an absurd artefact; each fails CLOSED on missing/malformed
    # input.
    if not check_bill_foots(bill):
        reasons.append(
            f"slc_6_7_billing_accuracy: declared total GBP "
            f"{bill.get('total_amount_gbp')} does not foot to the sum of line "
            f"items (commodity + non-commodity + standing charge + VAT) -- HELD"
        )
