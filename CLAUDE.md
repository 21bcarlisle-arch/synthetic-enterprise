...
**Phase AE COMPLETE (2026-06-29):** Customer Retention Offer Book -- 21 new tests (5,217 total). company/crm/customer_retention.py (new): OfferType (LOYALTY_DISCOUNT/TOU_REFERRAL/DUAL_FUEL_BUNDLE/PRICE_MATCH/ACCOUNT_REVIEW/NO_OFFER); OfferDeclineReason (NET_NEGATIVE/INSUFFICIENT_MARGIN); RetentionOffer (frozen; offer_value_gbp/max_spend_gbp=50%_of_margin; is_affordable/is_offer_made/expected_retention_value_gbp); CustomerRetentionBook (generate_offer selects type by driver: EV+RATE_SHOCK→TOU_REFERRAL zero-cost; elec-only→DUAL_FUEL_BUNDLE; RATE_SHOCK→PRICE_MATCH 8%; TENURE_SHORT/BASELINE→LOYALTY_DISCOUNT 5%; BILL_STRESS→ACCOUNT_REVIEW; net-neg→NO_OFFER; by_offer_type/total_retention_spend_gbp/retention_summary). Connects Phase AD (churn driver), Phase J (net margin), Phase X (ToU referral chain).
**Phase AD COMPLETE (2026-06-29):** Portfolio Churn Risk Book -- 34 new tests (5,196 total). company/crm/portfolio_churn_risk.py (new): ChurnRiskBand (CRITICAL≥50%/HIGH≥30%/MEDIUM≥15%/LOW); ChurnRiskDriver (RATE_SHOCK≥20%/BILL_STRESS≥£3k/TENURE_SHORT<2yr/BASELINE); CustomerChurnRisk (frozen; expected_loss_gbp=prob×revenue; is_at_risk for CRITICAL/HIGH); PortfolioChurnRiskBook (assess wraps churn_model.py estimate_churn_probability with all segment/fuel/hedge args; at_risk_customers/by_band/by_driver/total_revenue_at_risk_gbp/total_expected_loss_gbp/portfolio_churn_rate_pct/top_n_by_expected_loss/driver_breakdown/churn_risk_summary). Connects Phase J (profitability), Phase M (renewal), Phase AC (repricing trigger = potential churn driver).
