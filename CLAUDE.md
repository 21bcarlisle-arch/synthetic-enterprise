...
**Phase AM COMPLETE (2026-06-30):** Pricing Basis Risk -- 12 new tests (5,318 total). annual_report.py: _section_pricing_basis_risk(): basis_risk_terms company_fwd vs sim_fwd; year-by-year tariff error; HIGH OVER-PRICE/>15%/UNDER-PRICE/<-5%; post-crisis note. 2023/2025 HIGH OVER-PRICE (+18-33%).
**Phase AL COMPLETE (2026-06-30):** Counterfactual Retention Value -- 12 new tests (5,306 total). annual_report.py: _section_counterfactual_retention(): P_retain from retention_log; CF net benefit for positive-ETM no-offer churns; 5%/8% discount; correct-pass flag. £3,621 recoverable from 4 missed at £293 cost.
**Phase AK COMPLETE (2026-06-30):** Churn Root Cause Attribution -- 14 new tests (5,294 total). annual_report.py: _section_churn_root_cause(): rate shock/rate-vs-SVT/co vs sim per departure; blind misses/warned/crisis. 6 churns: £39,706 lost; 3 blind misses.
**Phase AJ COMPLETE (2026-06-30):** CRM Risk Triage -- 14 new tests (5,280 total). annual_report.py: _section_crm_intelligence(): CRITICAL/HIGH/MEDIUM/LOW bands; rate-vs-SVT; margin at risk; blind spots.
**Phase AI COMPLETE (2026-06-30):** EAC Drift Snapshot -- 10 new tests (5,266 total). annual_report.py: _section_eac_drift_snapshot() groups demand_estimation_log by customer, classifies drift significant/moderate/stable, infers cause (EV/solar/efficiency), table + trend. Connects Phase 23a/25a.
**Phase AH COMPLETE (2026-06-30):** Board Intelligence Pack -- 12 new tests (5,256 total). annual_report.py: _section_portfolio_intelligence_pack(): retention coverage/acceptance/margin; flex CAGR; churn peak year/net book movement; 4 board recs.
**Phase AG COMPLETE (2026-06-30):** Annual Report Flexibility Revenue Section -- 12 new tests (5,244 total). annual_report.py: _section_flexibility_revenue() renders year-by-year CM/DFS table; pre-DFS years labelled; portfolio total CM/DFS split; enrolled customer-years count. Connects Phase AF, Phase AA.
**Phase AF COMPLETE (2026-06-30):** DSR/Flexibility Revenue Integration -- 15 new tests (5,232 total). company/market/flexibility_revenue_book.py: FlexibilityRevenueBook (CM 2016+ £75/kW/yr; DFS 2022+ £4.5/MWh×20 events). EV: £555+£666/yr; EV+battery: £2,046/yr. Connects Phase AA, N, Q.
**Phase AE COMPLETE (2026-06-29):** Customer Retention Offer Book -- 21 new tests (5,217 total). company/crm/customer_retention.py (new): OfferType/OfferDeclineReason enums; RetentionOffer (frozen; max_spend=50% margin); CustomerRetentionBook (generate_offer by driver: EV+RATE_SHOCK→TOU_REFERRAL; RATE_SHOCK→PRICE_MATCH 8%; LOYALTY_DISCOUNT 5%; BILL_STRESS→ACCOUNT_REVIEW; net-neg→NO_OFFER; retention_summary). Connects Phase AD (churn), Phase J (margin), Phase X (ToU).
**Phase AD COMPLETE (2026-06-29):** Portfolio Churn Risk Book -- 34 new tests (5,196 total). company/crm/portfolio_churn_risk.py (new): ChurnRiskBand/ChurnRiskDriver enums; CustomerChurnRisk (frozen; expected_loss_gbp); PortfolioChurnRiskBook (at_risk_customers/by_band/by_driver/total_revenue_at_risk_gbp/portfolio_churn_rate_pct/churn_risk_summary). Connects Phase J, Phase M, Phase AC.
**Phase AC COMPLETE (2026-06-29):** Portfolio Repricing Action Book -- 24 new tests (5,162 total). company/crm/portfolio_repricing.py (new): RepricingPriority (CRITICAL/HIGH/MEDIUM/MONITOR; CRITICAL=urgent_drift+<=60d renewal; HIGH=urgent+<=180d; MEDIUM=material+far renewal); RepricingAction (frozen; tariff_delta_gbp_pa/estimated_margin_recovery_gbp_pa at 70% retention; is_urgent/is_actionable); PortfolioRepricingBook (plan_reprice recomputes recommended_tariff at existing unit rate on new AQ; critical_actions/actionable_actions/by_priority/total_margin_at_risk_gbp/total_expected_recovery_gbp/top_n_by_margin_recovery/portfolio_reprice_summary). Key finding: EV customer at 3,000→11,000 kWh = £2,000/yr tariff delta, £1,400/yr expected recovery. Connects Phase AB (drift detection), Phase M (renewal engine), Phase K (break-even).
**Phase AB COMPLETE (2026-06-29):** EAC Drift Assessor -- 35 new tests (5,138 total). company/crm/eac_drift_assessor.py (new): DriftDirection (INCREASED/DECREASED/STABLE ± 15%/10% thresholds); RenewalAction (URGENT_REPRICE/REPRICE_UPWARD/REPRICE_DOWNWARD/MAINTAIN; urgent=drift>30%); EACDriftAssessment (frozen; drift_kwh/drift_pct/drift_direction/likely_cause: ev_acquired/ashp_installed/solar_installed/ev_and_ashp_acquired; is_material); EACDriftBook (significant_increases/significant_decreases/renewal_reprice_candidates/urgent_reprice_candidates/mean_drift_pct/drift_summary). Key finding: EV customer at 3,000 kWh AQ now consuming 11,000 kWh = URGENT_REPRICE at renewal. Connects Phase C (EAC register), Phase H (multiplier at signing), Phase M (renewal conversion).
...
**Phase W COMPLETE (2026-06-29):** Gas Boiler Daily HDD Shape -- 13 new tests (5,003 total). simulation/gas_settlement.py: _GAS_BOILER_HEATING_FRACTION=0.70/_HDD_REF_ANNUAL from sim.weather_hdd; resi/SME daily consumption = AQ×0.70×(hdd_today/hdd_ref_annual) heating + AQ×0.30/365 DHW. I&C keeps Phase 60 monthly profile. run_phase2b.py: weather_factor_for_term removed (now internal). Annual total conserved with reference HDD; actual cold spells increase demand. Mirrors Phase I (ASHP electricity) pattern. Closes last major gas settlement accuracy gap.
**Phase V COMPLETE (2026-06-29):** ToU Migration Impact Scenario -- 16 new tests (4,990 total). company/pricing/tou_migration_scenario.py (new): MigrationScenario (frozen; migrated_count/retained_count/baseline_margin_gbp/post_migration_margin_gbp/margin_delta_gbp/revenue_delta_gbp/avg_customer_saving_gbp/is_margin_positive); ToUMigrationScenarioBook (run_scenario/compare_rates/best_supplier_scenario/portfolio_summary). Migration ordering: customers with lowest cross-subsidy migrate first (most likely to switch). Key finding: best supplier scenario for all-EV portfolio = 0% migration (flat tariff cross-subsidy never recovered under ToU); empty portfolio handled; crisis year delta larger. company/pricing/ev_cross_subsidy.py: CrossSubsidyRecord gains flat_revenue_gbp/tou_revenue_gbp/customer_saving_gbp properties. Closes ToU T-U-V chain: T=economics, U=per-customer ranking, V=portfolio migration scenario.
...
**Phase T COMPLETE (2026-06-29):** ToU Tariff Profitability Assessor -- 16 new tests (4,958 total). company/pricing/tou_tariff_assessor.py (new): DemandShapeClass (OVERNIGHT_HEAVY/STANDARD_FLAT/PEAK_HEAVY/WINTER_HEAVY); WholesaleBandRates (normal 35/42/65 GBP/MWh; crisis 280/380/520); ToURateStructure (default 7.5p/28.5p/45p overnight/standard/peak); ToUProfitabilityComparison (frozen; margin_delta_gbp/is_tou_beneficial_for_supplier/customer_saving_gbp/supplier_preferred_tariff); ToUTariffAssessorBook (assess/flat_preferred_accounts/tou_preferred_accounts/portfolio_summary). Key finding: EV customer (3,000 kWh normal market) earns supplier 4x more margin on flat tariff (746 GBP) vs ToU (189 GBP); peak-heavy customer ToU beneficial. Closes ToU economics gap enabled by Phase P overnight shape.
