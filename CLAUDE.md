...
**Phase A COMPLETE (2026-06-27):** Household physical model — 35 new tests (4,594 passing). `simulation/household.py` (new): `Household` frozen dataclass (PropertyType/BuildEra/HeatingSystem/BoilerAge/InsulationLevel enums; epc_consumption_multiplier/seasonal_flatness_factor/ev_annual_kwh/solar_annual_generation_kwh). `make_household()` maps existing customer records to representative UK profiles. `build_household_register()` covers all 18 customers. EPC multipliers calibrated to MHCLG EPC statistics (D=1.0 baseline). First item from Humansimulationlayer.md — Phase B (life events) follows. Discovery agent researching calibration data in parallel.
