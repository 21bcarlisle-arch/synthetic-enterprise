...
Current phase: Phase 3b APPROVED (2026-06-12, R^2=0.386 acceptable for Regime 2 distributional behaviour; caveat — not suitable for period-by-period price generation, see MASTER_BACKLOG.md). Phase 3c APPROVED (2026-06-12): weather engine accepted for Regime 2 — 0.952 cross-location temperature correlation (real vs synthetic) called "excellent". `sim/weather_engine.py` implements the two-pass model (national macro regime-switching AR1 + regional Cholesky deviations + half-hourly translation), fitted on real 2016-2025 daily weather for all 4 locations. See `docs/calibration/weather-engine.md`. Phase 2b (gas dual fuel): the `ANTHROPIC_API_KEY` blocker is resolved — per Rich's 2026-06-12 guardrail override, the risk committee agent is now routed through local Ollama (`qwen3:14b`) instead of the frontier. Context Handshake confirmed firing end-to-end (see `docs/observability/PHASE_2b_SUMMARY.md`); full 9.5-year re-run in progress.
...
- Phase 2b (gas dual fuel) — [REVIEW_GATE], ANTHROPIC_API_KEY escalation resolved (local-Ollama routing); pending full 9.5-year re-run completion and updated headline figures.
