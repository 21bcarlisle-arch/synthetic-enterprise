...
Current phase: Phase 3b APPROVED (2026-06-12, R^2=0.386 acceptable for Regime 2 distributional behaviour; caveat — not suitable for period-by-period price generation, see MASTER_BACKLOG.md). Phase 3c APPROVED (2026-06-12): weather engine accepted for Regime 2 — 0.952 cross-location temperature correlation (real vs synthetic) called "excellent". `sim/weather_engine.py` implements the two-pass model (national macro regime-switching AR1 + regional Cholesky deviations + half-hourly translation), fitted on real 2016-2025 daily weather for all 4 locations. See `docs/calibration/weather-engine.md`. Phase 2b (gas dual fuel): the `ANTHROPIC_API_KEY` blocker is resolved — per Rich's 2026-06-12 guardrail override, the risk committee agent is now routed through local Ollama (`qwen3:14b`) instead of the frontier. Full 9.5-year re-run with the Context Handshake actually firing complete: 160 wake-ups (all 2016 – early 2023), net margin **£13,970.60** (electricity £10,850.17 + gas £3,120.43), treasury £21,829.17 → £35,799.77. 2021 remains the only net-loss year but improved (-£1,621.33 vs -£2,002.62 with the handshake silent). See `docs/observability/PHASE_2b_SUMMARY.md`.
...
- Phase 2b (gas dual fuel) — [REVIEW_GATE], full re-run complete with Context Handshake active (see headline figures above); pending Rich's review.
...
- Phase 2b: `docs/observability/PHASE_2b_SUMMARY.md` — gas dual fuel (NBP SAP price feed, CV/CF conversion, C1g-C4g). Full 2016-2025 run with Context Handshake active: net margin £13,970.60 (electricity £10,850.17 + gas £3,120.43), treasury £21,829.17 → £35,799.77. 2021 only net-loss year (-£1,621.33). 160 Context Handshake wake-ups (all 2016 – early 2023; post-2023 regime keeps VaR below the trigger thereafter).
...
- **Phase 2b — Gas Dual Fuel** (`[REVIEW_GATE]`, LIVE): Earlier 2016-2025 dual-fuel run survived (net margin £16,799.11, treasury £21,829.17 → £38,628.27) but the Context Handshake never fired (every wake-up failed silently on the missing `ANTHROPIC_API_KEY`). **2026-06-12: Rich overrode the frontier-only guardrail** — the risk committee agent now calls local Ollama (`qwen3:14b`) instead of `anthropic.Anthropic()`, with the same validation clamps (no decrease, +0.10..+0.30) as the safety boundary regardless of model. Full 9.5-year re-run with the committee firing throughout is complete: **160 wake-ups** (all 2016 – early 2023, none after the post-2023 regime shift raises the stressed-VaR floor), net margin **£13,970.60** (vs £16,799.11 with the handshake silent — the committee's progressive hedge_fraction increases cost ~£2,829 in average-case margin, all on the electricity book), treasury £21,829.17 → £35,799.77, 2021 loss improved to -£1,621.33 (from -£2,002.62). See `docs/observability/PHASE_2b_SUMMARY.md` for the full breakdown.
