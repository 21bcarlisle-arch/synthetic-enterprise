...
def _parse_reset_minutes(line: str) -> int | None:
    ...
    for pattern in _RESET_PATTERNS:
        ...
        if target <= now:
            # The stated reset is in the past. Either the message is stale, or it names tomorrow.
            # A limit window is hours, never a day, so PAST means STALE and stale means run.
