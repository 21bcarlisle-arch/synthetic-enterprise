...
#: Where boot records live. Overridable by env SOLELY so a control can run the unit's OWN declared
#: `ExecStartPre` string as a real subprocess without writing real state — monkeypatching the module
#: global cannot reach a child process, and that child is the only thing that proves the DECLARED
#: command stamps. Nothing in production sets it.
BOOT_DIR = Path(os.environ["SE_BOOT_DIR"]) if os.environ.get("SE_BOOT_DIR") else (
    _REPO / "docs" / "observability" / ".daemon_boot")
...
def read_boot_ts(session: str) -> float | None:
    """WHEN this daemon last stamped, or None if never stamped/unreadable.

    THE DEFECT THIS EXISTS FOR, measured 2026-09-24. A stamp records a SHA but the reader had no
    way to ask whether the stamp describes the process running RIGHT NOW or a previous one. From
    2026-09-04 to 2026-09-24 the units' declared `ExecStartPre` was a no-op (the `__main__` below
    had been deleted), so every stamp on the box was days old and named a boot that had long since
    been replaced — and `loaded_code_drift` read those as valid, reporting 9 of 12 daemons `stale`
    continuously. A permanently-red detector is the failure `55899dc99` was written to abolish, and
    it is why the 2h48m window where `staging_watcher` ran without `reask()` was invisible: the
    session was already `stale` before it, during it and after it.

    Compared against the process's own start time, this separates "the code moved under a running
    daemon" from "nothing stamped this boot, so the question was never answered" — two conditions
    with opposite remedies that had one indistinguishable verdict.
    """
    try:
        value = json.loads((BOOT_DIR / f"{session}.json").read_text()).get("ts")
    except Exception:
        return None
...
if __name__ == "__main__":
    # THIS BLOCK IS THE WHOLE DEPLOYMENT MECHANISM, and it was deleted once already. Every unit
    # generated by `generate_units.py` declares
    #     ExecStartPre=-/usr/bin/python3 -m background.boot_sha <session>
    # and the leading `-` tells systemd to ignore the result. So when 3ecf355d8 removed this block
    # on 2026-09-04 while fixing an unrelated defect in `dirty_blobs`, the command kept exiting 0,
    # kept being declared in ten units, kept being described in three docstrings — and stamped
    # nothing for twenty days. Nothing could notice: no test ran the declared string, and the
    # consumer read the resulting week-old stamps as valid.
    #
    # `tests/background/test_boot_sha_deployment.py::test_the_units_own_declared_stamp_command_stamps`
    # runs this exact command as a subprocess, parsed out of the generated unit text rather than
    # retyped, so deleting or renaming it reds a named test instead of going quiet for three weeks.
    import sys
