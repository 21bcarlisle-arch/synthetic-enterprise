...
# ── the historical replay lives in `tools/standing_red_replay.py`, NOT here ──────────────────
#
# It reads the log through `commit_refusal_attribution.cycles` -- correctly, that module owns the
# log's vocabulary -- and `commit_refusal_attribution` imports `process_run_complete`, which
# `publish_scope.PUBLISH_PATH_SOURCES` names. Holding that import HERE gave the supervisor a path
# to the publish gate through `staging_rooms`'s RUNG-1 draw, putting the whole harness suite back
# inside the gate and wedging publishing for 725 minutes on 2026-09-05.
#
# So the direction is reversed and the module boundary carries it: this is the LIVE ledger and
# imports nothing; the replay is an ANALYSIS of it, reached only by a CLI and its tests, and
# imports this. See that module's docstring for the measured chain.
...
if __name__ == "__main__":
    ...
    except ModuleNotFoundError:  # launched as `python3 background/publish_standing_red.py`
        from _seat import refuse_if_foreign
    refuse_if_foreign("publish_standing_red")
    import sys
    sys.exit(main())
