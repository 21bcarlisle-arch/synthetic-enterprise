...
#:
#: The segments the domestic switching curve governs. Moved here from
#: `simulation/customer_events._bill_scale_for` on 2026-09-09 because the rule is a property of
#: THIS CURVE, not of the caller that happened to hold it first: `_savings_to_rate` is calibrated
#: on DESNZ/Ofgem HOUSEHOLD switching, so who may be scaled by their own bill is the curve's own
#: question. It had one caller and one copy; when `tools/couple_value_based_pricing` needed the
#: same gate the choice was a second copy or a shared home, and a second copy of a calibration
#: boundary is how this repository has previously ended up with one legal rule in five places.
_DOMESTIC_SEGMENTS = {"resi"}


def bill_scale_for(segment: str | None, bill_gbp: float | None) -> float | None:
