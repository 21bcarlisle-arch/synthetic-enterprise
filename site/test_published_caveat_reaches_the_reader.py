...
WHY THIS FILE MOVED FROM `site/proof/` TO `site/` (2026-08-20, rung-1c BLOCKING draw).
It did not move: it was DELETED, and this is its restoration at a path that survives.
`03dd8c49e` retired eleven pages by removing their directories wholesale, `site/proof/`
among them, and this control went with the directory. But its subject was never the
`/proof/` PAGE -- it is `site/data/proof.json`, the published data artefact, which that
commit kept and the live door still renders from `/harness/`. So the deletion was
collateral: the page died and the thing this guards did not.

THE COST OF THAT COLLATERAL, measured rather than asserted. For five days
`detection_resolution_caveat` had no consumer outside its own module and unit test, so the
code/artefact gap this file exists to close was unguarded again -- the finding's BLOCKING 3
("the control has no caller and never runs") regressed past its own diagnosis into "the
control does not exist". A control filed under a PAGE inherits that page's mortality; this
one is filed under the site ROOT because `pytest site/` collects the whole tree and the
broad site-lane trigger (site/data/**) is precisely the change set that strands a
