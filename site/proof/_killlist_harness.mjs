// Render harness for the Proof-door CONTROL KILL-LIST panel (R15).
// Extracts the inline <script> from index.html, executes it against a minimal
// DOM + fetch stub, invokes renderKilllist(data), and prints the resulting
// element contents as JSON so a test can assert on the RENDERED pixels (R11).
//
// Usage: node _killlist_harness.mjs <index.html>   (data JSON on stdin)
...
const out = {};
for (const id of ["kl-intro", "kl-kpis", "kl-alarms", "killlist"]) {
  const e = elements[id];
