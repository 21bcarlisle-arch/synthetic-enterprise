...
def test_live_repeated_c9_prediction_is_one_row():
    ...
    # on four consecutive days rendered as four rows implying four predictions.
    gpd = _gpd()
    led = gpd._predictions_ledger()
    body = _render()["pred-body"]["innerHTML"]
    rows = led["renewal"]["rows"]
    assert len(rows) == 1, rows
    assert rows[0]["relog_count"] == 4, rows[0]
    assert body.count("renewal 2025-06-29") == 1, "the C9 prediction still repeats"
    assert "re-logged 4&times;" in body, body
