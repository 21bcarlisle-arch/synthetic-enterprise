// Render harness for the Method door's Activity-Cost section (G11).
// Extracts the page's inline <script>, runs it against a minimal DOM + inert
// fetch stub, invokes the page's OWN renderActivityCost() against the supplied
// activity_cost.json, and prints every captured ac-* element's contents as JSON
// so a test can assert on the RENDERED pixels (R11), not the source string.
//
// Usage: node _render_activity_cost_harness.mjs <index.html>   (activity_cost.json on stdin).
...
sandbox.renderActivityCost(d);

const ids = ["ac-framing", "ac-guardrail", "ac-kpis", "ac-taxonomy", "ac-trend", "ac-basis"];
const out = {};
for (const id of ids) {
  const e = elements[id];
