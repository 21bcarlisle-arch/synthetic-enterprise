...
def test_the_live_feed_carries_it_too():
    ...
    """
    # `published_json` is fail-closed on a subject that is not in the index, which is the same
    # finding the hand-rolled `is_file()` guard here used to report and a stricter one: a feed
    # that exists on disk and has never been landed is a page no reader has met.
...
def test_no_subject_of_this_file_is_read_from_the_working_tree():
    ...
    """
    refuse_working_tree_reads(__file__, (DOOR_REL, FEED_REL, CAPS_REL, ARMS_REL, DD_ARMS_REL))
