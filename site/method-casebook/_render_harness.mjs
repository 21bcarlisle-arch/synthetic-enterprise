// Render harness for the Method+Simplified door (site/method-casebook/index.html).
// Extracts the inline <script>, runs it against a minimal DOM + inert fetch stub,
// sets the module-level DATA the page's own callback sets, invokes the render
// functions against the supplied method_casebook.json, and prints every captured
// element's contents as JSON so a test can assert on the RENDERED pixels (R11).
//
// Usage: node _render_harness.mjs <index.html>   (method_casebook.json on stdin).
...
// The page callback sets a module-level DATA before rendering the register; mirror it.
sandbox.DATA = d;
sandbox.renderCasebook(d.casebook || {});
sandbox.renderRegister(d.simplified || {});
sandbox.renderBuild(d);

const ids = [
  "hero-framing", "roles", "routing-framing", "tiers", "no-fable",
