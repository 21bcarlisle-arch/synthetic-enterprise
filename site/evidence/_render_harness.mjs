// Render harness for the Evidence door (site/evidence/index.html).
// Extracts the inline <script>, runs it against a minimal DOM + inert fetch stub,
// invokes the page's own renderEvidence() against the supplied moap_evidence.json,
// and prints every captured element's contents as JSON so a test can assert on the
...
const sandbox = { document, fetch, console, Date, Number, String, Object, Math, JSON, Array, isFinite, setTimeout() {} };
sandbox.window = sandbox;
vm.createContext(sandbox);
vm.runInContext(code, sandbox);
sandbox.renderEvidence(d);

const ids = ["stamp", "summary", "nodes", "sources"];
const out = {};
for (const id of ids) {
  const e = elements[id];
