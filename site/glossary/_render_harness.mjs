// Render harness for the Glossary door (site/glossary/index.html).
// Extracts the inline <script>, runs it against a minimal DOM + inert fetch stub,
// invokes the page's own renderGlossary() against the supplied glossary.json, and
// prints every captured element's contents as JSON so a test can assert on the
// RENDERED pixels (R11), not the source string.
//
// Usage: node _render_harness.mjs <index.html>   (glossary.json on stdin).
...
sandbox.renderGlossary(d);

const ids = ["hero-h", "hero-p", "filters", "count", "glist"];
const out = {};
for (const id of ids) {
  const e = elements[id];
